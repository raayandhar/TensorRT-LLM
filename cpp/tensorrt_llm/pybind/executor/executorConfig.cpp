...
void initConfigBindings(pybind11::module_& m)
{
    ...
    auto cacheTransceiverConfigSetstate = [](py::tuple const& state)
    {
        ...
        auto config = tle::CacheTransceiverConfig(
            state[0].cast<std::optional<tle::CacheTransceiverConfig::BackendType>>(), 
    ...
    py::class_<tle::CacheTransceiverConfig>(m, "CacheTransceiverConfig")
        .def(py::init([](std::optional<tle::CacheTransceiverConfig::BackendType> backend,
                        std::optional<size_t> maxTokens,
        ...
        }), py::arg("backend") = std::nullopt, py::arg("max_tokens_in_buffer") = std::nullopt, py::arg("kv_transfer_timeout_ms") = std::nullopt)
        .def_property(
        ...
        .def_property("kv_transfer_timeout_ms", 
        ...
        .def("setKvTransferTimeoutMs", [](tle::CacheTransceiverConfig& self, int64_t timeoutMs) {
            self.setKvTransferTimeoutMs(std::chrono::milliseconds(timeoutMs));
        })
